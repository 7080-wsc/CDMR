...
def main():
    ...
    writer = SummaryWriter(log_dir=os.path.join(ckpt_root, 'tb', f'seed{seed}'))

    for epoch in range(1, args['base']['n_epochs']+1):
        train(model, dataLoader['train'], optimizer, loss_fn, epoch, metrics, writer)

        if args['base']['do_validation']:
            valid_results = evaluate(model, dataLoader['valid'], loss_fn, epoch, metrics, writer, phase='valid')
        ...
        test_results = evaluate(model, dataLoader['test'], loss_fn, epoch, metrics, writer, phase='test')
...
def train(model, train_loader, optimizer, loss_fn, epoch, metrics, writer=None):
    y_pred, y_true = [], []
    loss_dict = {}
    ...
    for cur_iter, data in enumerate(train_loader):
        ...
        if cur_iter == 0:
            for key, value in loss.items():
                loss_dict[key] = value.item() if hasattr(value, 'item') else float(value)
        else:
            for key, value in loss.items():
                loss_dict[key] += value.item() if hasattr(value, 'item') else float(value)
    ...
    if writer is not None:
        writer.add_scalar('train/loss_total', loss_dict.get('loss', 0), epoch)
        writer.add_scalar('train/mae',        loss_dict.get('mae', 0), epoch)
        writer.add_scalar('train/corr',       results.get('Corr', 0), epoch)


def evaluate(model, eval_loader, loss_fn, epoch, metrics, writer=None, phase='valid'):
    loss_dict = {}
    ...
    if writer is not None:
        writer.add_scalar(f'{phase}/mae',  loss_dict.get('mae', 0), epoch)
        writer.add_scalar(f'{phase}/corr', results.get('Corr', 0), epoch)
